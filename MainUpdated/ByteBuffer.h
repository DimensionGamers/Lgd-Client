...
class ByteBuffer
{
    public:
        ...
        // constructor
        ByteBuffer() : _rpos(0), _wpos(0), default_t(0)
        {
            _storage.reserve(DEFAULT_SIZE);
        }

        ByteBuffer(size_t reserve) : _rpos(0), _wpos(0), default_t(0)
        {
            _storage.reserve(reserve);
        }

        ByteBuffer(ByteBuffer const& right) : _rpos(right._rpos), _wpos(right._wpos),
            _storage(right._storage), default_t(0) { }
		...
		bool is_all_read() const
		{
			return _rpos >= _storage.size();
		}
    protected:
        size_t _rpos, _wpos;
